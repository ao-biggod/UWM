...
class ResNetImageEncoder(nn.Module):
    ...
    def __init__(self, num_views: int, embed_dim: int):
        ...
        # Feature dim before global pooling (layer4 output channels)
        bn2 = self.model.layer4[-1].bn2
        self._resnet_feat_dim = getattr(bn2, "num_features", None) or bn2.num_channels
        # Projection for token-level spatial features
        self.token_proj = nn.Linear(self._resnet_feat_dim, embed_dim)
    ...
    def encode_tokens(self, imgs: torch.Tensor):
        """Return per-frame spatial tokens for cross-attention.

        imgs: (B, V, C, T, H, W) — same shape as forward().
        Returns: (B, N_spatial_tokens, embed_dim)
            where N_spatial_tokens = V * T * H' * W'
            with H',W' = spatial grid after ResNet layer4 (before pooling).
        """
        B, V = imgs.shape[:2]
        imgs = rearrange(imgs, "b v c t h w -> (b v t) c h w")
        imgs = self.norm(imgs)
        # Manual forward through ResNet layers, stopping before global pooling
        x = self.model.conv1(imgs)
        x = self.model.bn1(x)
        x = self.model.relu(x)
        ...
        x = self.model.layer4(x)  # [B*V*T, C_feat, H', W']
        x = rearrange(x, "n c h w -> n (h w) c")       # [B*V*T, H'*W', C_feat]
        tokens = self.token_proj(x)                     # [B*V*T, H'*W', embed_dim]
        tokens = rearrange(tokens, "(b v t) n c -> b (v t n) c", b=B, v=V)
        return tokens
