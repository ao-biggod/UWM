...
class UWMObservationEncoder(nn.Module):
    def __init__(
        ...
        if use_low_dim:
            low_dim_size = sum(
                shape_meta["obs"][key]["shape"][-1] for key in self.low_dim_keys
            )
            self.lowdim_token_proj = nn.Linear(low_dim_size, embed_dim)
    ...
    def encode_obs_memory(self, curr_obs_dict: dict):
        """Return token-level obs memory for cross-attention conditioning.

        Produces spatial image tokens + per-frame low-dim tokens,
        unlike encode_curr_obs() which returns a single pooled vector.
        """
        imgs = self.apply_transform(curr_obs_dict)
        img_tokens = self.img_encoder.encode_tokens(imgs)  # [B, N_img, embed_dim]

        tokens_list = [img_tokens]
        if self.use_low_dim:
            low_dims = [curr_obs_dict[key] for key in self.low_dim_keys]
            low_dims = torch.cat(low_dims, dim=-1)  # [B, T, low_dim_size]
            low_tokens = self.lowdim_token_proj(low_dims)  # [B, T, embed_dim]
            tokens_list.append(low_tokens)
        ...
        if self.use_language:
            ...
            tokens_list.append(lang_feats.unsqueeze(1))  # [B, 1, embed_dim]

        return torch.cat(tokens_list, dim=1)  # [B, N_total, embed_dim]
