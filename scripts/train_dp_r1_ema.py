#!/usr/bin/env python3
"""R1: DP-only + EMA (Exponential Moving Average).
Minimal modification of train_pusht_dp.py — adds EMA with decay=0.9999.

Usage:
  python scripts/train_dp_r1_ema.py --num-steps 20000 --batch-size 64 --device cuda:0 \
    --output-dir outputs/ablate_r1
"""
...
def save_checkpoint(model, train_set, step, save_path, ema_model=None):
    ...
    if ema_model is not None:
        ckpt["ema_model"] = {k: v.cpu().clone() for k, v in ema_model.shadow.items()}
...
def main():
    ...
    parser.add_argument("--ema", type=float, default=0.9999,
                        help="EMA decay rate (default: 0.9999, official DP value)")
    ...
    # EMA
    ema_model = EMAModel(model, decay=args.ema) if args.ema > 0 else None
    if ema_model is not None:
    ...
    for step in range(start_step, args.num_steps):
        ...
        if step > 0 and args.save_every > 0 and step % args.save_every == 0:
            ...
            save_checkpoint(model, train_set, step, save_path, ema_model)
            latest_path = os.path.join(args.output_dir, "latest.pt")
            save_checkpoint(model, train_set, step, latest_path, ema_model)
    ...
    save_checkpoint(model, train_set, args.num_steps - 1, final_path, ema_model)
    log_file.close()
